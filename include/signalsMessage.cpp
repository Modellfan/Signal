...
signalsMessage::signalsMessage(uint32_t id)
{
...
void signalsMessage::updateSignals(const CANMessage& msg) {
  if (msg.id == _id) {
    for (int i = 0; i < CFG_MAX_SIGNALS; i++) {
      if (_signals[i]) {
...
void Message::removeSignal(Signal *signal) {
...
int8_t Message::findSignal(Signal *signal) {
    for (int i = 0; i < CFG_MAX_SIGNALS; i++) {
